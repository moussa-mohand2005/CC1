...
export class RegisterParentComponent implements OnInit {
  ...
  private initializeForm(): void {
    this.parentForm = this.fb.group({
      ...
      ecole_des_enfants: ['', Validators.required],
      nombre_enfants: ['', [
        Validators.required, 
        Validators.min(1)
      ]],
      ...
      role_academique: ['', Validators.required]
    });
  }
    ...
    try {
      ...
      if (snapshot.exists()) {  
        ...
        this.foundChildren = Object.entries(users)
          ...
          .filter(user => {
            const nameMatch = 
              user.firstName?.toLowerCase().trim() === this.childSearch.prenom.toLowerCase().trim() &&
        ...
        if (this.foundChildren.length === 0) {
          ...
          console.log('Found children:', this.foundChildren);
        }
      }
    } catch (error) {
  ...
  private validateSearchFields(): boolean {
    ...
    if (!this.childSearch.nom?.trim()) {
      errors.push('Last name');
    }
    if (!this.childSearch.prenom?.trim()) {
      errors.push('First name');
    }
    if (!this.childSearch.dateNaissance) {
      errors.push('Birth date');
    }

    if (errors.length > 0) {
      this.errorMessage = `Please fill in the following fields: ${errors.join(', ')}`;
      return false;
    }
  ...
  getFieldError(fieldName: string): string {
    ...
    if (control?.errors) {
      ...
      if (control.errors['pattern']) {
        switch (fieldName) {
          case 'telephone': return 'Phone number must be at least 8 digits.';
          case 'cin': return 'National ID must be 8 digits.';
          case 'adresse.code_postal': return 'Postal code must be 4 or 5 digits.';
